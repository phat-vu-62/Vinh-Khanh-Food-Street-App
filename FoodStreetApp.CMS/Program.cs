...
using (var scope = app.Services.CreateScope())
{
    try
    {
        ...
        Console.WriteLine("[DB] Initializing Database schema...");
        
        dbContext.Database.EnsureCreated();
        
        // Ensure PoiSyncActions table exists
        ...
        // Ensure all required columns exist on the Pois table
        var poiColumns = new[]
        {
        ...
        {
            try {
                dbContext.Database.ExecuteSqlRaw("ALTER TABLE \"Pois\" ADD COLUMN IF NOT EXISTS \"" + col + "\" text;");
            } catch { /* Suppress: Likely already exists or PG compatibility sync */ }
        }

        // Ensure DurationSeconds on UserHistories
        try {
    ...
    {
        Console.WriteLine($"[DB Error] Fatal error during startup migrations: {ex.Message}");
        // We log and continue, allowing the app to start and show errors in the UI rather than crashing with 134
    }
}
